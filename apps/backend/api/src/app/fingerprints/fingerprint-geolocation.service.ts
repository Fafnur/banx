...
export class FingerprintGeolocationService {
  ...
  async save({ visitor, data, process }: FingerprintDto<GeolocationCoordinates | null>): Promise<FingerprintGeolocationEntity | null> {
    ...
    return !record
      ? await this.geolocationEntityRepository.save(
          this.geolocationEntityRepository.create({ visitor, fingerprint, data: data ?? {}, process })
        )
      : null;
  }
}
