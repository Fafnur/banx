...
export class RegistrationFinishController {
  ...
  async createUser(@Param() params: { process: string }): Promise<void> {
    await this.registrationUserService.finish(params.process);

    return this.registrationProcessService.finishStep(params.process, RegistrationStepType.Finish).then();
  }
}
