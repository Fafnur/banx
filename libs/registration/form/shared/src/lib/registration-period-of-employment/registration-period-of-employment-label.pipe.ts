...
export const REGISTRATION_PERIOD_OF_EMPLOYMENT_LABELS: Record<RegistrationPeriodOfEmployment, string> = {
...
@Pipe({
  name: 'registrationPeriodOfEmploymentLabel',
})
export class RegistrationPeriodOfEmploymentLabelPipe implements PipeTransform {
  transform(periodOfEmployment: RegistrationPeriodOfEmployment | null, empty: string = ''): string {
    if (periodOfEmployment && !REGISTRATION_PERIOD_OF_EMPLOYMENT_LABELS[periodOfEmployment]) {
      console.warn(`Unknown RegistrationPeriodOfEmployment: ${periodOfEmployment}`);
    }

    return periodOfEmployment ? REGISTRATION_PERIOD_OF_EMPLOYMENT_LABELS[periodOfEmployment] ?? periodOfEmployment : empty;
  }
}
