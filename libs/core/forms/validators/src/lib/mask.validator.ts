 ...
 */
export function maskMinLengthValidator(length: number, regExp: string | RegExp = /[^a-zA-Z0-9]+/g): ValidatorFn {
 ...
 * @param regExp String or reg exp for replace
 */
export function maskMaxLengthValidator(length: number, regExp: string | RegExp = /[^a-zA-Z0-9]+/g): ValidatorFn {
  return (control: AbstractControl): { [key: string]: any } | null => {
    let forbidden = false;
    if (control.value && typeof control.value === 'string') {
      forbidden = control.value.replace(regExp, '').length > length;
